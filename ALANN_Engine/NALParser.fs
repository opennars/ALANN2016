...
let pextset = setBetweenStrings "{" "}" pterm (sort >> ExtSet)
let pextset_ws = pextset .>> ws
let pintset = setBetweenStrings "[" "]" pterm (sort >> IntSet)
...
opp.AddOperator(InfixOperator   ("&&",   ws, 3, Associativity.Left, fun x y -> reduce(And(sort [x; y]))))
opp.AddOperator(InfixOperator   ("||",   ws, 3, Associativity.Left, fun x y -> reduce(Or(sort [x; y]))))
opp.AddOperator(InfixOperator   (",",    ws, 3, Associativity.Left, fun x y -> Seq([x; y])))
opp.AddOperator(InfixOperator   (";",    ws, 3, Associativity.Left, fun x y -> Par(sort [x; y])))
...
opp.AddOperator(InfixOperator   ("*",    ws, 2, Associativity.Left, fun a b -> reduce(Prod([a; b]))))
...
let pjudgement   = pipe3 (opt ptense) statement (opt ptruth)  (fun t s tv -> makeSentence(makeTaskKey(SentenceType.Judgement, s, (if Option.isNone t then Eternal else tenseToOccurs t.Value)), (if Option.isNone tv then Some (1.0f, 0.9f) else tv), None))
let pquestion    = pipe3 (strWS "?") (opt ptense) statement (fun _ t s -> makeSentence(makeTaskKey(SentenceType.Question, s, if Option.isNone t then Eternal else tenseToOccurs t.Value), None, None))
let pgoal        = pipe3 (strWS "!") statement (opt pdesire) (fun _ s dv -> makeSentence(makeTaskKey(SentenceType.Goal, s, Eternal), None, (if Option.isNone dv then Some (1.0f, 0.9f) else dv)))
let pquest       = pipe2 (strWS "??") statement (fun _ s -> makeSentence(makeTaskKey(SentenceType.Quest, s, Eternal), None, None))
