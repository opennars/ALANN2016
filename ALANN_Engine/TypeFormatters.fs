...
type TypeFormatter() =
    ...
    static member TermToUTF8(t : Term) = 
        let rec tp term =
            ...
            match term with
            | Not(s)        -> "Not,"  + tp s + ")"
            | And(lst)      -> "And," + separateList lst "," + ")"
            | Or(lst)       -> "Or," +  separateList lst "," + ")"
    ...
    static member Occurence =
        function
        | i when i = Eternal                                           -> ""
        | i when i > (SystemTime.Now + Parameters.Parameters.DURATION) -> ":/:"
        | i when i < (SystemTime.Now - Parameters.Parameters.DURATION) -> ":\:"
        | _                                                            -> ":|:"
    ...
    static member Stamp(stamp : Stamp) = 
        sprintf "{Created %s; Occurs %s; SC %1.2f; Origin %A; Freq %f; Trail %s; Derivation %s}" 
    ...
    static member StampHeader(stamp : Stamp) = 
        sprintf "{Stamp Header [Created %s; Occurs %s; SC %1.2f; Origin %A; Freq %f]" 
            (stamp.Created.ToString())
            (stamp.Occurs.ToString())
            stamp.SC 
            (TypeFormatter.Origin stamp.Origin) 
    ...
    static member Sentence(sentence :  Sentence) = 
        ...
        sprintf "%s %s %s %s %s"
                (prefix sentence) 
                (TypeFormatter.Occurence sentence.Key.Occurs)
