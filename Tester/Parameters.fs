module Parameters
    ...
    type Parameters private() =
        ...
        static member val ACTIVATION_THRESHOLD                = 0.25f with get, set       // Concept Activation Threshold - when threshold is above priority concept is activated
        static member val SPIKE_ACTIVATION                    = 0.25f with get, set       // Amount of activation contributed by a spike
        static member val INFERENCE_TASKS_PER_CYCLE           = 5 with get, set           // Number of tasks to select for each inference task
        static member val SPIKE_THRESHOLD                     = 0.25f with get, set       // Amount of activation contributed by a spike
        static member val INFERENCE_THRESHOLD                 = 0.00f with get, set       // Minimum bellief priority to be accepted as active belief
        ...
        static member val TRAIL_LENGTH                        = 3 with get, set           // maximum length allowed for inference trail within stamp
        ...
        static member val CONCEPT_CAPACITY                    = 500 with get, set         // Size of concept pool in Working memory
        static member val BELIEF_CAPACITY                     = 200 with get, set         // Size of Belief pool within each concept
        static member val TASK_CAPACITY                       = 5 with get, set           // Size of Task pool within each concept
        static member val INFERENCE_ACTORS                    = 50 with get, set           // Number of threads to run the inference step - one concept per thread
        ...
        static member val DECAY_RATE                          = 0.05f with get, set       // Decay rate tuning parameter - smaller = slower decay rate (used in Attention.Decay)
        ...
        let parseParameters(node:XElement) =
            let f (a : XAttribute) =
                try
                    ...
                    | "SpikeActivation", value                -> Parameters.SPIKE_ACTIVATION                  <- float32(value)
                    | "InferenceThreshold", value             -> Parameters.INFERENCE_THRESHOLD               <- float32(value)
                    | "SpikeThreshold", value                 -> Parameters.SPIKE_THRESHOLD                   <- float32(value)
